...
def generate_newpk_refset(ref_keys, update_keys):
    ...
    # for appending 0s, the amount depends on the digit, lim 10^6
    for newseq_record in range(max_number_pk + 1, max_number_pk + number_keys_generate + 1):
...
def Main(fastafiletest, referencefile, mappingfile):
    ...
    # newversion_ref_seq df with pk, seq only
    newversion_ref_seq = drop_column_fromdf(newversion_ref_seq_no_marker, length_label)
    append_new_zotus_onrefset = newversion_ref_seq.merge(
        sequences_to_upload, on=list(newversion_ref_seq), how="outer")  # appending new zotulist
    append_new_zotus_onrefset.columns = ["refsequence_pk", "sequence"]  # new header for merging set
    # keep first duplicates only! This means that the original PK is kept
    append_new_zotus_onrefset.drop_duplicates(subset=["sequence"], inplace=True, keep="first")
    ...
    fetch_existing_zotus_onrefset = existing_zotus_onrefset[existing_zotus_onrefset.duplicated(
        subset=["sequence"], keep='first')][["refsequence_pk", "sequence"]]  # keep pk of existing sequences
    fetch_new_zotus_onrefset = existing_zotus_onrefset[existing_zotus_onrefset.duplicated(
        subset=["sequence"], keep='last')][["refsequence_pk", "sequence"]]  # keep zotus of existing sequences
    # create the mapping, while keeping matching sequences
    remove_refseq_duplicate = fetch_existing_zotus_onrefset.merge(
        fetch_new_zotus_onrefset, on="sequence")  # df which contains the mapping of existing sequences
    # this contains the mapping new_zotus | refset pk_existing seq
    mapping_existing_zotu_seq = remove_refseq_duplicate.drop("sequence", axis=1)
    ...
    append_new_zotus_onrefset['refsequence_pk'] = append_new_zotus_onrefset['refsequence_pk'].replace(
        updated_key_zotu_mapping)  # new ref set, append attributes
    ...
    for key, value in updated_key_zotu_mapping.items():  # NOW APPEND MARKER AND LENGTH
        print(key, value)
    ...
    updated_key_zotu_mapping = pandas.DataFrame(updated_key_zotu_mapping.items(
    ), columns=["zotu_id", "refsequence_pk"])  # convert dictionary in dataframe
    ...
    # MERGE PREVIOUS KEYS TO NEW KEYS FOR MAPPING TABLE
    df_mapping_zotu_pk = pandas.concat([updated_key_zotu_mapping, mapping_existing_zotu_seq])
    ...
    # NOW MERGE THE PREVIOUS MAPPING WITH NEW MAPPING
    df_new_mapping = pandas.concat([prev_mapping_pk_srr, df_mapping_pk_srr_zotu])
